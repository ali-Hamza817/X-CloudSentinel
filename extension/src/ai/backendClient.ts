...
export class BackendClient {
    ...
    constructor() {
        ...
        this.apiKey = config.get<string>('apiKey', 'X-CloudSentinel-Research-2026');
    }

    /**
     * Update base URL and API key from configuration
     */
    ...
    /**
     * Get shared headers for all requests
     */
    private getHeaders() {
        return {
            headers: {
    ...
    public async getExplanation(text: string): Promise<SHAPExplanation | null> {
        try {
            const response = await axios.post(`${this.baseUrl}/explain`, { text }, { 
                timeout: 20000,
                ...this.getHeaders()
            });
    ...
    public async getRemediation(text: string, title: string): Promise<any | null> {
        try {
            const response = await axios.post(`${this.baseUrl}/remediate`, { text, title }, { 
                timeout: 30000,
                ...this.getHeaders()
            });
            return response.data;
    ...
    public async getAdvancedAnalysis(text: string, filePath: string): Promise<AdvancedScanResult | null> {
        try {
            const response = await axios.post(`${this.baseUrl}/analyze-advanced`, { text, filePath }, { 
                timeout: 30000,
                ...this.getHeaders()
            });
            return response.data;
